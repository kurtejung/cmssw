  ...
  JetCorrectionService           (iConfig.getParameter<std::string>         ("JetCorrections"))
{
  ...
  csCandToken_ = mayConsume<reco::PFJetCollection>(mInputCsCandCollection);
...
void JetAnalyzer_HeavyIons::bookHistograms(DQMStore::IBooker & ibooker, edm::Run const & iRun, edm::EventSetup const &iSetup) 
{
  ...
  TH2F *h2D_etabins_forRho = new TH2F("etabinsForRho","",etaBins_, edge_pseudorapidity, 500,0,1000);
  TH2F *h2D_ptBins_forRho = new TH2F("ptBinsForRho","",500,0,1000,500,0,1000);
  TH2F *h2D_centBins_forRho = new TH2F("centBinsForRho","",200,0,200,500,0,1000);
  ...
  if(isPFJet){
	...
	mSumPFPtEtaDep[ieta] = ibooker.book1D(histoName.c_str(),Form("Sum PFPt in the eta range %.3g to %.3g",edge_pseudorapidity[ieta],edge_pseudorapidity[ieta+1]),1000,-1*range,range);
    }
    ...
    for(int ieta=0; ieta<etaBins_; ieta++){ 
        ...
        for(int ipt=0; ipt<ptBins_; ipt++){
                mSubtractedEFrac[ipt][ieta]= ibooker.book1D(Form("subtractedEFrac_JetPt%d_to_%d_etaBin%d",ptBin[ipt],ptBin[ipt],ieta),"subtracted fraction of CS jet",50,0,1);
                mSubtractedE[ipt][ieta]= ibooker.book1D(Form("subtractedE_JetPt%d_to_%d_etaBin%d",ptBin[ipt],ptBin[ipt],ieta),"subtracted total of CS jet",200,0,600);
        }
    }
...
void JetAnalyzer_HeavyIons::analyze(const edm::Event& mEvent, const edm::EventSetup& mSetup)
{
  ...
  std::cout << "found bigged vtx" << std::endl;
  int nGoodVertices = 0;
  ...
  std::cout <<" filling vtx histo" << std::endl;
  ...
  edm::Handle<reco::PFJetCollection> csCandidates;
  ...
  std::cout << "grabbing CS stuff" << std::endl;
  ...
  if(std::string("Cs")==UEAlgo){
	  ...
	  std::cout << "rhoSize: "<< rhoSize << std::endl;
	  double rhoRange[rhoSize];
	  for(int irho=0; irho<rhoSize; irho++){ std::cout << " rho: "<< etaRanges->at(irho) << std::endl; rhoRange[irho]=etaRanges->at(irho); }
	  double yaxisLimits[501];
	  for(int ibin=0; ibin<501; ibin++) yaxisLimits[ibin] = ibin*2;
	  std::cout << "resizing histograms" << std::endl;
  ...
  std::cout << "getting jet tokens " << std::endl;
  ...
  std::cout << "calo jet loop " << std::endl;
  if(isCaloJet){
    ...
    std::cout << "found " << caloCandidates->size() << " calo candidates" << std::endl;
    ...
    std::cout << " neta bins: "<< nedge_pseudorapidity-1 << std::endl;
    ...
    std::cout << "eta bins: "<< etaBins_ << std::endl;
    for(int ieta=0; ieta<etaBins_; ieta++){
	mSumCaloPtEtaDep[ieta]->Fill(SumCaloPt[ieta]);
    }

    std::cout << "looping over eta again?? "<< std::endl;
    for(size_t  k = 0;k<nedge_pseudorapidity-1;k++){
    ...
    std::cout << "finalizing eta" << std::endl;
  ...
  std::cout << "reached pf jet loop" << std::endl;
  if(isPFJet){
    ...
    if(std::string("Cs")==UEAlgo){
	    const reco::PFJetCollection *csCandidateColl = csCandidates.product();

	    for(unsigned iCScand=0; iCScand<csCandidateColl->size(); iCScand++){
		const reco::PFJet pfCandidate = csCandidateColl->at(iCScand);
		int ieta=0;
  ...
  for (unsigned ijet=0; ijet<recoJets.size(); ijet++) {
	...
	nJet_40++;
      ...
      if(std::string("Cs")==UEAlgo){
	      int ipt=0, ieta=0;
	      while(recoJets[ijet].pt()>ptBin[ipt] && ipt<ptBins_-1) ipt++;
	      while(recoJets[ijet].eta()>edge_pseudorapidity[ieta] && ieta<etaBins_-1) ieta++;
	      mSubtractedEFrac[ipt][ieta]->Fill((recoJets[ijet].energy()-recoJets[ijet].pileup())/recoJets[ijet].energy());
